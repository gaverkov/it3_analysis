...
\begin{bem}[Die Rolle des Formalismus] 
...
%\begin{bsp} 
%	$\sum_{k=1}^\infty k/3^k$ ist nach dem Quotientenkriterium konvergent.
%\end{bsp} 
...
\begin{thm}[Wurzelkriterium] % Seite 65, Theorem 3.33 in Rudin 
	Eine  Reihe $\sum_{k=1}^\infty a_k$ mit reellwertigen Gliedern und $q := \lim_{k \to \infty} |a_k|^{1/k} \in [0,\infty)$ ist bei $0 \le q <1 $ konvergent und bei $1 < q  \le \infty$ divergent. 
\end{thm} 
...
\begin{aufg}
	Die folgenden Funktionen gehören zu den Funktionen, die  in künstlichen neuronalen Netzwerken als die sogenannten Aktivierungskfunktionen benutzt werden: 
	\begin{align*}
			\text{Tangens hyperbolikus} & & \tanh (x)  = \frac{e^x - e^{-x}}{e^x + e^{-x}}
\\ \text{Logistische Funktion} & & \frac{1}{1 +e^{-x}}
\\			\text{Softplus} & & \ln(1+ e^x)  
\\ \text{sigmoid linear unit} & & \frac{x}{1+ e^{-x}}
\\  \text{Gauß-Glocke} & & e^{-x^2}
	\end{align*} 
Berechnen Sie die Ableitungen der aufgelisteten Funktionen.
\end{aufg} 
...
\begin{bem}[Wieso existiert das $\xi$ aus dem Mittelwertsatz?]
...
\begin{bem}[Allgemeine Aufgaben/Fragestellungen] \ 
	\begin{itemize}
...
\begin{thm}[Ordnung der Approximation durch Taylor-Entwicklung]  
...
\begin{bsp}
	Die Taylor-Approximation $\sqrt{1+x} = 1 + \frac{1}{2} x + O(x^2)$ für $x \to 0$ wird in der Optik bei der Herleitung der Gleichung einer sphärischen Linse benutzt (Abhängigkeit der Brennweite von den beiden Radien -- \url{https://en.wikipedia.org/wiki/Thin_lens}).
\end{bsp} 
...
\begin{bem}
	...
	\[
		f(x) = f(a) + f'(a)(x-a) + \frac{1}{2} f''(a) (x-a)^2 + O((x-a)^3), \qquad x \to a. 
	\]
...
\begin{aufg}
	Rechnen Sie $T_\infty(x)$ bzgl. der Entwicklungsstelle $a=0$ für die  Funktionen $e^x, \cos x, \sin x$ und $\frac{1}{1-x}$ und für $\ln x$ bzgl. der Entwicklungsstelle $a=1$ aus: 
\end{aufg} 
...
\begin{bsp} 
	Die Potenzreihe $\sum_{k=1}^\infty \frac{(-1)^k}{k} x^k$ hat den Konvergenzbereich $(-1,1]$.  
...
\begin{bsp} 
	Der Konvergenzradius von $\sum_{k=0}^\infty \frac{1}{k!} x^k$ ist $\infty$. Das kann man auf die folgenden Weise ausrechnen. Einerseits gilt 
	\[
		\lim_{k \to \infty} \frac{1/k!}{1/(k+1)!} = \lim_{k \to \infty} (k+1) = \infty. 
	\]
	Anderseits gilt auch 
	\begin{equation} \label{k-te-wurzel-fakultaet}
		\lim_{k \to \infty} k!^{1/k} = \infty. 
	\end{equation} 
	Den vorigen Grenzwert kann man so ausrechnen: $k!$ ist das Produkt der Zahlen $1$ bis $k$. Betrachten wir zuerst den Fall, dass $k$ gerade ist. Dann sind die Zahlen $k/2,\ldots, k$ in diesem Produkt nach unten durch $k/2$ abschätzbar. Da es mindestens $k/2$ Zahlen sind, die wir durch $k/2$ abschätzen, gilt $k! \ge (k/2)^{k/2}$, sodass $k!^{1/k} \ge \sqrt{k/2}$ erfüllt ist. Für ein ungerades $k$ kann man die Zahlen $(k+1)/2,\ldots, k$ durch $k/2$ nach unten abschätzen. Es sind ebenfalls mindestens $k/2$ Zahlen, sodass man auch in diesem Fall die Abschätzung $k!^{1/k} \ge \sqrt{k/2}$ erhält. Da man $\sqrt{k/2} \to \infty$ bei $k \to \infty$ hat, ist \eqref{k-te-wurzel-fakultaet} tatsächlich erfüllt. 
\end{bsp} 
...
\begin{thm}
	Sei $\sum_{k=0}^\infty a_k (x-c)^k$ Potenzreihe und sei $\rho$ Konvergenzradius  dieser Reihe mit $0< \rho < \infty$. Dann ist die Funktion $f(x) := \sum_{k=0}^\infty a_k (x-c)^k$ im Bereich $(c-\rho,c+\rho)$ beliebig oft differenzierbar. Darüber hinaus gilt:  
	\begin{itemize} 
	\item Die	$i$-te Ableitung $f^{(i)} (x)$ mit $i \in \N$ lässt als eine Potenzreihe durch das gliederweise Differenzieren der Reihe $\sum_{k=0}^\infty a_k (x-c)^k$ darstellen. Das heißt, die Reihe $\sum_{k=0}^\infty ( a_k (x-c)^k)^{(i)}$ ist absolut konvergent in $(-\rho + c,c+\rho)$ mit
	\[
		f^{(i)}( x) = \sum_{k=0}^\infty \bigl( a_k (x-c)^k \bigr)^{(i)}.
	\]
	\item Man hat $a_k = \frac{f^{(k)}(c)}{k!}$ für alle $k \in \N$. Mit anderen Worten ist $f(x) = T_\infty(x)$ für die Taylor-Reihe von $f$ bzgl. des Entwicklungpunkts $c$.  
	\end{itemize} 
\end{thm} 
...
\begin{bem}
	Die rationale Funktion $f(z):=\frac{1}{(2-z)(3-z)}$ hat Polstellen in $2$ und $3$ ind er komplexen Ebene $\C$. Die Entfernung der Polstelle, die am nächsten zur $0$ liegt bestimmt den Konvergenzradius. 
	\begin{center}
	\includegraphics[width=0.5\textwidth]{pics/complex_plot_rational.pdf} 
	\end{center} 
	Im Bild beschreibt der Farbton das Argument und die Helligkeit den Betrag von $f(z)$. 
\end{bem} 
...
\begin{bsp} 
	...
	\begin{center}
	\includegraphics[width=0.5\textwidth]{pics/complex_plot_rational_2.pdf} 
\end{center} 
...
\begin{bsp}
	Durch die Anwendung des Satzes vom Fubini bei der Berechnung des (ueigentlichen) Integrals
	\[
		\int_{[0,\infty)^2} e^{-x y} \sin x \dd x
	\]	
	lässt sich das sogenannte Dirichlet-Integral 
	\[
...
\begin{bsp}
	Durch die Anwendung von Polarkoordinaten beim Berechnen des (uneigentlichen) zweidimensionalen Integrals 
	\[
		\int_{\R^2} e^{ - \frac{1}{2} (x^2 + y^2) } \dd x \dd y
	\]
	lässt sich das sogenannte Gauß-Integral 
	\[
		\int_{-\infty}^\infty e^{-\frac{1}{2} x^2}  \dd x = \sqrt{ 2 \pi}
	\]
	bestimmen. Das zeigt, dass die Funktion $\frac{1}{\sqrt{2\pi}} e^{-\frac{1}{2} x^2}$ Dichte einer Zufallsvariablen ist (einer standardnormalverteilten  Variablen). 
\end{bsp} 
